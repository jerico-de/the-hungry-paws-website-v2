...
async function getPets(req, res, next) {
  try {
    ...
    const pets   = await db.collection("pets").find({ userId: new ObjectId(userId) }).sort({ createdAt: -1 }).toArray();
...
async function addPet(req, res, next) {
  try {
    ...
    if (!name || !breed || !age || !gender) throw new ValidationError("All fields are required");
    if (age < 0 || age > 30)                throw new ValidationError("Invalid age — must be between 0 and 30");
    if (!["male", "female"].includes(gender.toLowerCase())) throw new ValidationError("Gender must be male or female");
...
async function updatePet(req, res, next) {
  try {
    ...
    if (result.modifiedCount === 0) throw new NotFoundError("Pet not found");
...
async function deletePet(req, res, next) {
  try {
    ...
    if (result.deletedCount === 0) throw new NotFoundError("Pet not found");
